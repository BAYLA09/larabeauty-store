...
import { X, ShoppingBag, Plus, Star } from "lucide-react";
...
function formatQuantity(item: CartItem) {
  const total = item.offerQuantity * item.qty;
  if (total === 1) return "علبة واحدة";
  if (total === 2) return "علبتين";
  return `${total} علب`;
}

function getRecommendations(cartItems: CartItem[], limit = 2) {
  const inCart = new Set(cartItems.map((i) => i.slug));
  const related = new Set<string>();
  for (const item of cartItems) {
    const product = getProductBySlug(item.slug);
    product?.relatedSlugs.forEach((s) => related.add(s));
  }
  const recommended = products.filter(
    (p) => related.has(p.slug) && !inCart.has(p.slug)
  );
  if (recommended.length >= limit) return recommended.slice(0, limit);
  const extras = products.filter(
    (p) => !inCart.has(p.slug) && !recommended.includes(p)
  );
  return [...recommended, ...extras].slice(0, limit);
}
          ...
          {items.length === 0 ? (
            <div className="py-12 text-center">
              <ShoppingBag
                ...
                aria-hidden
              />
              <p className="mt-4 font-arabic text-sm font-bold text-foreground">
                السلة فاضية
              </p>
              <p className="mt-1 text-xs text-muted">
                اختاري علكتك المفضّلة وابدئي الروتين
              </p>
              <Link
                href="/#products"
                onClick={closePanel}
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="shrink-0 text-left">
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              remove(item.productId, item.offerId)
                            }
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <p className="mt-0.5 text-[11px] font-bold text-primary">
                                {formatStartingPrice(offer.price)}
                              </p>
                            </div>
                            <button
                              type="button"
